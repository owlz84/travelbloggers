...
func (u *UserModel) PasswordUpdate(id int, currentPassword string, newPassword string) error {
	var hashedCurrentPassword []byte
	selectStmt := "SELECT hashed_password from users where id = ?"
	err := u.DB.QueryRow(selectStmt, id).Scan(&hashedCurrentPassword)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return ErrNoRecord
		} else {
	...
	_, err = u.DB.Exec(updateStmt, string(hashedNewPassword), id)
	return err
}
